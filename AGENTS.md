...
- Every target rebuilds the image first. `make test` picks up local edits
  ...
  (the image WORKDIR). `make run` / `make run_detached` are mount-free on
  purpose (production-like).
...
- Packages: `commands/` (slash commands), `reactions/` (emoji reactions),
  `chat_statistics/`, `clients/` (external APIs), `media_downloads/`
  (yt-dlp based public-media downloader: URL classification, extraction,
  rate limiting, env-driven settings).
